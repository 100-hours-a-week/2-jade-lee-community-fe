...
async function loadPosts() {
    try {
        const response = await fetch('http://43.201.148.13:8000/api/posts', {
            method: 'GET',
            credentials: 'include',
        });
        ...
        data.data.posts.forEach(post => { 
            ...
            const imageUrl = `http://43.201.148.13:8000${post.author.image_url}`;
            const postElement = document.createElement("div");
            postElement.classList.add("post");
            postElement.onclick = async () => {
                try {
                    const response = await fetch(`http://43.201.148.13:8000/api/posts/${post.post_id}/`, {
                        method: 'POST',
                        headers: {
