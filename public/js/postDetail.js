...
async function editCommentSend(post_id, comment_id) {
    try {
        ...
        const response = await fetch(`http://43.201.148.13:8000/api/posts/${post_id}/comments/${comment_id}`, {
            method: 'PATCH',
            headers: {
...
async function loadPosts() {
    try {
        const response = await fetch(`http://43.201.148.13:8000/api/user`, {
            method: 'GET',
            credentials: 'include',
        });
    ...
    try {
        ...
        document.getElementById('postImage').src = data.data.postImage ? `http://43.201.148.13:8000${data.data.postImage}` : ''
        document.getElementById('author_image').src = data.data.author.profileImage ? `http://43.201.148.13:8000${data.data.author.profileImage}` : '/images/profile_img.png'
                    ...
                    <div class="comment-header">
                        <div class="writer-profile">
                            <button class="profile">
                                <img width="35px" src="${comment.author.profileImage ? `http://43.201.148.13:8000${comment.author.profileImage}` : '/images/profile_img.png'}" />
                            </button>
...
async function editPost() {
    ...
    try {
        const response = await fetch(`http://43.201.148.13:8000/api/posts/${postId}/like`, {
            method: 'POST',
...
async function addComment() {
    ...
    try {
        const response = await fetch(`http://43.201.148.13:8000/api/posts/${postId}/comments`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function deleteComment(commentId){
    try {
        const response = await fetch(`http://43.201.148.13:8000/api/posts/${postId}/comments/${commentId}`, {
            method: 'DELETE',
            credentials: 'include', 
        });
        if (response.status === 200) {
